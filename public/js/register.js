...
const regExLetter = /^[A-Z]+$/i //Exprecion regular SOLO LETRAS
const regExEmail = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/
const regExPass = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,15}$/;
...
$('name').addEventListener('blur' , function(){  // Hace evento al salir del imput
    switch (true) {
        ...
        case this.value.trim().length <2 || this.value.trim().length > 255:
            $('errorNombre').innerHTML = 'El nombre debe tener como minimo 2 caracteres'
            this.classList.add('is-invalid')
            break
        default:
...
$('surname').addEventListener('blur' , function(){  // Hace evento al salir del imput
    switch (true) {
        case !this.value.trim():
        ...
        case !regExLetter.test(this.value.trim()):
            ...
            break
        case this.value.trim().length <2 || this.value.trim().length > 255:
            $('errorApellido').innerHTML = 'El apellido debe tener como minimo 2 caracteres'
            this.classList.add('is-invalid')
            break
        default:
            this.classList.remove('is-invalid')
...
$('email').addEventListener('blur' , async function(){  // Hace evento al salir del imput
    switch (true) {
        case !this.value.trim():
...
$('pass').addEventListener('blur' , async function(){  // Hace evento al salir del imput
    switch (true) {
        case !this.value.trim():
            $('errorPass').innerHTML = 'Debes ingresar tu contraseña'
            this.classList.add('is-invalid')
            break;
        case !regExPass.test(this.value.trim()):
            $('errorPass').innerHTML = 'La contraseña debe tener entre 6 y 15 caracteres, un numero, una mayuscula y un caracter especial'
            this.classList.add('is-invalid')
            break
        default:
            this.classList.remove('is-invalid')
...
$('pass2').addEventListener('blur' , async function(){  // Hace evento al salir del imput
    switch (true) {
        case !this.value.trim():
